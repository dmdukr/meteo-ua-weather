...
import { createWarningText, deepMerge, normalizeDate } from "./helpers";
...
import "./components/wfc-monthly-grid";
...
export class WeatherForecastCard extends LitElement {
  ...
  @state() private _effectTestCondition?: string;
  @state() private _selectedMonthlyDate?: string;
  ...
  private _fullHourlyForecastData?: ForecastAttribute[];
  ...
  public static async getConfigElement() {
    return document.createElement("meteo-ua-weather-forecast-card-editor");
  }
  ...
  public setConfig(config: WeatherForecastCardConfig): void {
    ...
    this.config = deepMerge({}, DEFAULT_CONFIG, migratedConfig) as WeatherForecastCardConfig;
    this._currentForecastType = this.config.default_forecast || "daily";
    this._effectTestCondition = this.config.debug_condition;
  }
  ...
  protected shouldUpdate(changedProperties: PropertyValues): boolean {
    return (
      ...
      changedProperties.has("_effectTestCondition") ||
  ...
  protected render(): TemplateResult | typeof nothing {
    ...
    // Effect test mode: override condition
    const effectStateObject = this._effectTestCondition
      ? { ...stateObject, state: this._effectTestCondition } as WeatherEntity
      : stateObject;
    
    // Force all effects enabled during test mode
    const effectConfig = this._effectTestCondition
      ? { ...this.config, show_condition_effects: true }
      : this.config;
    ...
    return html`
      <ha-card>
        ${effectConfig.show_condition_effects
          ? html`<wfc-animation-provider
              .hass=${this.hass}
              .weatherEntity=${effectStateObject}
              .config=${effectConfig}
        ...
        <div class="wfc-container">
          ...
          ${this.config.monthly_grid?.show !== false
            ? html`<wfc-monthly-grid
                .hass=${this.hass}
                .config=${this.config}
                .dailyForecast=${this._dailyForecastData || []}
                .hourlyForecast=${this._fullHourlyForecastData || this._hourlyForecastData || []}
                .selectedDate=${this._selectedMonthlyDate}
                @monthly-day-select=${this._onMonthlyDaySelect}
              ></wfc-monthly-grid>`
            : nothing}
        </div>
        ${effectConfig.show_condition_effects && effectStateObject.state.includes("lightning")
          ? html`<div class="wfc-lightning-flash"></div>`
          : nothing}
      </ha-card>
    `;
  }

  private _onMonthlyDaySelect(ev: CustomEvent): void {
    const detail = ev.detail;
    if (!detail?.forecast?.datetime) return;

    const clickedDate = detail.forecast.datetime;
    const clickedDateStr = new Date(clickedDate).toDateString();
    this._selectedMonthlyDate = clickedDate;

    if (!this._fullHourlyForecastData?.length) return;

    const wasAlreadyHourly = this._currentForecastType === "hourly";
    this._currentForecastType = "hourly";

    const targetForecast = this._fullHourlyForecastData!.find(
      (h) => new Date(h.datetime).toDateString() === clickedDateStr
    );

    if (targetForecast) {
      const targetIdx = this._fullHourlyForecastData!.indexOf(targetForecast);
      const neededSlots = targetIdx + 24;
      if (this._hourlyForecastData && neededSlots > this._hourlyForecastData.length) {
        this._hourlyForecastData = this._fullHourlyForecastData!.slice(0, neededSlots);
      }

      const doScroll = () =>
        this.scrollToForecastItem(targetForecast, wasAlreadyHourly ? "smooth" : "instant");
  ...
  private processForecastData() {
    ...
    // Keep full hourly data for monthly day-click scroll
    this._fullHourlyForecastData = this._hourlyForecastData ? [...this._hourlyForecastData] : undefined;
  ...
  private scrollToForecastItem(
    ...
    // Search through shadow DOM of child components
    let scrollContainer: HTMLElement | null = null;
    const children = this._forecastContainer.querySelectorAll("*");
    for (const child of children) {
      if (child.shadowRoot) {
        scrollContainer = child.shadowRoot.querySelector(".wfc-scroll-container") as HTMLElement;
        if (scrollContainer) break;
      }
    }
    // Fallback to direct search
    if (!scrollContainer) {
      scrollContainer = this._forecastContainer.querySelector(".wfc-scroll-container") as HTMLElement;
    }
